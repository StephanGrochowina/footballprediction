...
@NamedQueries({
        ...
        @NamedQuery(name = TotalGoalsOddGroupMapping.QUERY_LOAD_TOTAL_GOALS_FOR_ODD_GROUP_AND_THRESHOLD,
                query = "SELECT avg(m.expectedTotalGoals) "
                        ...
                        + "and ROUND(sqrt("
                        + "POWER(m.oddBelowThreshold.oddValue - :oddValueBelowThreshold, 2) + "
                        + "POWER(m.oddAboveThreshold.oddValue - :oddValueAboveThreshold, 2) "
                        + "), 4) = (SELECT MIN("
                        + "ROUND(sqrt("
                        + "POWER(p.oddBelowThreshold.oddValue - :oddValueBelowThreshold, 2) + "
                        + "POWER(p.oddAboveThreshold.oddValue - :oddValueAboveThreshold, 2) "
                        + "), 4)"
                        + ") FROM TotalGoalsOddGroupMapping p where p.threshold = :threshold) "
                        + "group by ROUND(sqrt("
                        + "POWER(m.oddBelowThreshold.oddValue - :oddValueBelowThreshold, 2) + "
                        + "POWER(m.oddAboveThreshold.oddValue - :oddValueAboveThreshold, 2) "
                        + "), 4)"
